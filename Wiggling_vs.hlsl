//--------------------------------------------------------------------------------------
// Wiggling Sphere Model Vertex Shader
//--------------------------------------------------------------------------------------
...
SimplePixelShaderInput main(BasicVertex modelVertex)
{
    ...
    float4 normal = float4(modelVertex.normal, 0);
    ...
    float4 worldNormal = mul(gWorldMatrix, normal);
    worldNormal = normalize(worldNormal);

    worldPosition.x += sin(modelPosition.y + wiggle) * 0.5f;
    worldPosition.y += sin(modelPosition.x + wiggle) * 0.5f;

    worldPosition += worldNormal * (sin(wiggle) + 1.0f) * 0.1f;
    output.projectedPosition = mul(gViewProjectionMatrix, worldPosition);
